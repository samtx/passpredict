# Test passpredict/models.py

import passpredict.models as models
...
def test_Satellite():
    ...
    satellite = models.Satellite(id=satid, name=name)
...
def test_Satellite_fail():
    ...
    satid = 25544
    name = "International Space Station"
    satellite = models.Satellite(satid, name)
    assert satellite.id == satid
    assert satellite.name == name
...
def test_Tle():
    ...
    tle = models.Tle(tle1=tle1, tle2=tle2, epoch=epoch, satellite=satellite)
...
def test_Point():
    ...
    elevation = 84.1
    range_ = 3200.214
    point = models.Point(datetime=dt, azimuth=azimuth, elevation=elevation, range=range_)
...
def test_Location():
    ...
    h = 88
    name = 'Austin, Texas'
    austin = models.Location(lat=lat, lon=lon, h=h, name=name)
