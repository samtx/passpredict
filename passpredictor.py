...
def parabolic_blending(t, p):
    ...
    See Vallado, Appendix C.5.1 for finding cubic roots 
    """
    from math import sqrt, atan2, cos, degrees, radians, pi
...
def alfano_approx(t, lat, lon, h=0.):
    ...
    f_elev = np.arccos(np.cos(elev_lim)/R_sat) - elev_lim - np.arccos(np.cos(elev)/R_sat) + elev_lim
...
if __name__ == "__main__":
    ...
    try:
        ...
        tend = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(days=dt_days)
        (tt, azs, alts, rr, rrsat) =  razel_from_t(tend, sec_per_query=sec_per_query, sat=sat)
        ...
        np.savez('iss_razel.npz', tt=tt, azs=azs, alts=alts, rr=rr, rrsat=rrsat)
    ...
    try:
        ...
        # el_lower_lim_rad = np.radians(el_lower_lim)
        # cos_el_lim = np.cos(el_lower_lim_rad)
        # alts_rad = np.radians(alts_lofi)
        # elev_fn1 = np.degrees(np.arccos(np.cos(alts_rad)/rr[0:-1:lofi_step]))
        # elev_fn2 = -np.degrees(alts_rad) 
        # elev_fn3 = (-1)*np.degrees(np.arccos(cos_el_lim/rr[0:-1:lofi_step]))
        # elev_fn4 = np.degrees(el_lower_lim_rad)*np.ones(tt_lofi.size)
        # elev_fn = elev_fn1 + elev_fn2 + elev_fn3 + elev_fn4
        elev_fn = alts_lofi - el_lower_lim
        np.savez('elev_fn.npz', elev_fn=elev_fn, tt=tt, alts=alts, rr=rr, el_lower_lim=el_lower_lim, tt_lofi=tt_lofi, alts_lofi=alts_lofi)
    ...
    for i in range(1, tt_lofi.size-2):
        if (np.product(elev_fn[i-1:i+1]) > 0) and (np.product(elev_fn[i:i+2]) > 0) \
            ...
            continue
        a_el = compute_alphas(elev_fn[i-1],elev_fn[i],elev_fn[i+1],elev_fn[i+2])
    ...
    # ax.plot(tt, elev_fn3, 'o-', markersize=markersize, label='Elev. Fn3.')
    # ax.plot(tt, elev_fn4, 'o-', markersize=markersize, label='Elev. Fn4.')
    # ax.vlines(trise, ymin, ymax, label='Rise times')
    # ax.vlines(tset, ymin, ymax, label='Set times')
    ax.set_xlabel('Minutes since epoch')
    ax.set_ylabel('Elevation Angle')
    # ax.axis.set_major_locator(days)
